...
/*
 * Multiplies input with MDS matrix (matrix mutiplication, both size m).
 */
template MultiplyMDS(m, MDS) {
    signal input in[m];
    signal output out[m];

    var mds_line[m];
    for (var i = 0; i < m; i++) {
        var temp = in[0] * MDS[i][0];
        for (var j = 1; j < m; j++) {
            temp += in[j] * MDS[i][j];
        }
        out[i] <== temp;
...
template PoseidonPermutationFullRound(m, i, MDS, round_constants) {
    ...
    component pow5[m];
    component m_mds = MultiplyMDS(m, MDS);
 ...
 */
template PoseidonPermutationPartialRound(m, i, MDS, round_constants) {
    signal input in[m];
    signal output out[m];
    ...
    // + round constants * alpha (only the last state element)
    pow5.in <== in[0] + round_constants[i*m];
    m_mds.in[0] <== pow5.out;

    // + round constants (all but last) * MDS
    for (var j = 1; j < m; j++) {
        m_mds.in[j] <== in[j] + round_constants[i*m + j];
    }
    for (var j = 0; j < m; j++) {
        out[j] <== m_mds.out[j];
    }
