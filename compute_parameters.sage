...
OUTPUT = None
try:
    OUTPUT = open(sys.argv[8], 'x')
except KeyError:
    print('[!] No output_filename given!')
    exit(1)
...
if FIELD == 1 and len(sys.argv) < 8:
    ...
    # e.g. 0xa7, 0xFFFFFFFFFFFFFEFF, 0xa1a42c3efd6dbfe08daa6041b36322ef
...
for i in range(0, NUM_CELLS):
    if FIELD == 0:
        matrix_string += str([entry.integer_representation() for entry in MDS[i]])
    elif FIELD == 1:
        matrix_string += str([int(entry) for entry in MDS[i]])
    if i < (NUM_CELLS-1):
        matrix_string += ","
matrix_string += "]"
...
print("n:", FIELD_SIZE)
print("t:", NUM_CELLS)
print("N:", (FIELD_SIZE * NUM_CELLS))
print("Number of round constants:", len(round_constants))
print("Result Algorithm 1:\n", algorithm_1(MDS, NUM_CELLS))
print("Result Algorithm 2:\n", algorithm_2(MDS, NUM_CELLS))
print("Result Algorithm 3:\n", algorithm_3(MDS, NUM_CELLS))



# print to file
pprint('pragma circom 2.0.4;')
pprint()
pprint('function get_round_constants() {')
pprint('return', end=' ')
pprint(round_constants, end=';\n')
pprint('}')
pprint()

pprint('function get_mds() {')
pprint('return', end=' ')
pprint(matrix_string, end=';\n')
pprint('}')
pprint()

pprint(f'function m() {{ return {NUM_CELLS}; }}')
pprint()

pprint(f'function Rf() {{ return {R_F_FIXED // 2}; }}')
pprint()

pprint(f'function Rp() {{ return {R_P_FIXED}; }}')
